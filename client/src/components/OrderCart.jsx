  ...
  return (
    <div className=' flex flex-col gap-4 bg-gray-50 h-screen w-full relative'>
      ...
      <h3 className="text-center text-gray-600 py-2">Thanh toán</h3>
       ...
       <span className="max-sm:text-sm text-red-500 font-semibold">${isChecked?.reduce((initValue, currentValue) => {
       ...
       <div className="fixed bottom-0 w-[30%] max-sm:w-full items-center justify-center flex  mx-auto h-16 bg-slate-100 shadow-md">
         ...
         <div className="flex gap-2 items-center line-clamp-1  ">
          <span className="max-sm:text-xs text-black text-lg font-semibold">Tổng tiền:</span>
          <span className="max-sm:text-sm text-red-500 font-semibold text-xl">${isChecked?.reduce((initValue, currentValue) => {
            const totalProductPrice = currentValue?.product?.price * currentValue?.quantity;
            return initValue + totalProductPrice;
          },0)}</span>
         </div>
          <button className="px-8 py-2 max-sm:text-xs rounded-xl bg-red-500 text-white" onClick={() => onCreateAddressOrder()} type="submit">Đặt hàng</button>
          </div>
         </div>
    </div>
  )
