...
import { apiDeleteOrderById, apiGetOrder, apiGetOrderByIdOrder, apiGetOrderByShop, apiGetOrderSuccess, apiUpdateOrder } from "@/services/orderServer";
...
function OrderlistDone() {
  ...
  const fetchOrders = async (page = 1) => {
    setLoading(true);
    try {
      const res = await apiGetOrderSuccess({ page, limit: 10 });
      console.log(res)
      setProductList(res.orders1);
      setStore(res?.stores1)
      setTotalPages(res.totalPages); // Set total pages for pagination
      setLoading(false);
    } catch (error) {
      console.error(error);
          ...
          <div className="card">
            ...
            {isLoading ? (
              ...
              <>
                ...
                {/* Pagination Controls */}
                <div className="flex justify-between items-center mt-4 px-4 pb-6">
                  <button
