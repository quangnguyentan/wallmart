  ...
  return (
    <div className="bg-gray-50 h-screen overflow-x-scroll scrollbar-hide">
      ...
      <KeyboardArrowLeftIcon
             sx={{ fontSize  : `${isMobile ? "40px" : "50px"}`}}
        ...
        <div className="flex items-center relative w-[90%] px-2">
            ...
            <input
                  type="text"
                  className="w-full h-11 max-sm:h-8 rounded-full bg-gray-100  pl-11 text-lg outline-none placeholder:text-orange-600 placeholder:font-medium max-sm:text-xs"
    ...
    <Tabs defaultValue="all" className="w-full h-full ">
      <div className="w-full flex flex-col z-40 px-2 bg-white outline-none overflow-hidden ">
        ...
        <TabsTrigger 
                 value="all" 
                 className={`max-sm:text-sm text-2xl ${activeTab === 'all' ? `background-custom ` : 'text-gray-500'}`} 
        ...
        <TabsTrigger 
                className={`max-sm:text-sm text-2xl ${activeTab === 'sell' ? ` background-custom ` : 'text-gray-500'}`} 
                value="sell" 
            ...
            <TabsTrigger 
                className={`max-sm:text-sm text-2xl cursor-default ${activeTab === 'price' ? 'background-custom  ' : 'text-gray-500'}`} 
                value="price"
                onClick={() => setActiveTab("price")}
            >
               <div className="flex items-center justify-center gap-2">
                 <span className="cursor-pointer"> Giá cả</span>
               <SwapVertOutlinedIcon className="cursor-pointer"/>
               </div>
            </TabsTrigger>
